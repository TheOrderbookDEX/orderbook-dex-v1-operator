...
export class Orders {
    ...
    add(owner: string, orderType: OrderType, price: bigint, amount: bigint) {
        if (amount == 0n) {
    ...
    totalAmount(orderType: OrderType) {
        return this.prices(orderType).reduce((subtotal, price) =>
            subtotal + price * this.available(orderType, price)
        , 0n);
    }
